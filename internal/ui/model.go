...
func (m Model) viewRequestBuilder() string {
	...
	b.WriteString("\n")
	b.WriteString(RenderFooter("F1: help • F2: send • F3: load • F4: history • F5: database • F6: env • h: headers • b: body • q: query • s: save • x: cURL"))
...
func (m Model) viewDatabaseQueryEditor() string {
	...
	buttons := RenderButton("Execute (F5)", true) + "  "
	...
	b.WriteString("\n\n")
	b.WriteString(RenderFooter("F5: execute • Ctrl+S: save query • Esc: back"))
